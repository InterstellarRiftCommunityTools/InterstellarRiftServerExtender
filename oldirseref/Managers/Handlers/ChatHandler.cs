...
namespace IRSE.Managers.Handlers
{
	...
	public class ChatHandler
	{
		...
		public ChatHandler(Game.Server.ControllerManager controllerManager)
		{
			try
			{
				LogManager.MainLog.WriteLineAndConsole("IRSE: Loading ChatManager...");
		...
		public bool ParseCommand(Player ply, string msg)
		{
			...
			if (Config.Settings.GameAdminsSteamID.Contains(ply.ID))
			{
				...
				if (givematch.Success && givematch.Groups.Count == 4)
				{
					...
					ClSvEconomics.ResourceTypes resourceTypes;
					try
					{
		...
		protected void OnPacketChatMessage(RPCData data)
		{
			try
			{
				Player player =	ServerInstance.Instance.Handlers
					.PlayerHandler.Players.ByConnectionID[(long)data.OriginalMessage.sender.connectionID];
				...
				{
					LogManager.MainLog.WriteLineAndConsole("'" + name + "' used command " + message );
					return;
				}
				...
				LogManager.ChatLog.WriteLineAndConsole(String.Format("[Chat]{0}: {1}", name, message));
